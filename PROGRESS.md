...
**已完成：**
- [x] 文件操作栏（单击即现）+ 内联重命名（分离扩展名）
- [x] 移动/复制对话框（面包屑导航+列表选目标）
- [x] 右键菜单全部功能可用
- [x] 键盘快捷键 + 撤销提示 + 列表自动刷新
- [x] 双面板浏览 + 收藏夹
- [ ] 前端: 右键菜单所有功能可用
...
**已完成：**
- [x] AIRenameDialog：AI分析→推荐3个命名→选择→批量应用
- [x] 文件操作栏「AI 重命名」按钮（单文件）
- [x] 批量操作栏「AI 重命名」按钮（多文件）
- [x] 右键菜单「AI 重命名」
- [x] 无API Key时安全兜底建议
- [x] 后端AI服务集成DeepSeek V4（需设DEEPSEEK_API_KEY环境变量）

**待完成：**
- [ ] 命名模板管理器UI (TemplateManager)
- [ ] 命名模式历史UI (PatternHistory)
