...
import { DeleteOutlined, CopyOutlined, SwapOutlined, RobotOutlined } from '@ant-design/icons'
...
export default function BatchActionsBar() {
  ...
  const handleDelete = () => {
    Modal.confirm({
      title: '确认删除', content: `删除选中的 ${selectedPaths.length} 个文件？可撤销。`, okText: '删除', okType: 'danger', cancelText: '取消',
      onOk: async () => {
        ...
        clearSelection(); message.success(`已删除 ${selectedPaths.length} 个文件`)
      },
    })
  }
    ...
    <>
      <div className="batch-bar">
        ...
        <Space>
          ...
          <Button size="small" icon={<RobotOutlined />} onClick={() => setAIRenameOpen(true)}>AI 重命名</Button>
      ...
      <AIRenameDialog open={aiRenameOpen} filePaths={selectedPaths} onClose={() => setAIRenameOpen(false)} />
    </>
  )
}
